...
class ProviderRuntimeConfigSourceContractTest(unittest.TestCase):
    ...
    def test_providers_ui_keeps_custom_and_readonly_sections_with_write_only_key_input(self) -> None:
        ...
        self.assertIn("provider-config-drawer", builtin_source)
        self.assertIn("function builtinConfigDrawer", builtin_source)
        ...
        self.assertIn("drawerLayer.hidden = false", builtin_source)
        self.assertIn("drawerLayer.hidden = true", builtin_source)
        self.assertIn("document.body.appendChild(drawerLayer)", builtin_source)
        self.assertIn("drawerLayer.remove()", builtin_source)
        self.assertIn("window.addEventListener('hashchange', routeHandler, { once: true })", builtin_source)
        self.assertIn("document.documentElement.classList.add('provider-drawer-open')", builtin_source)
        self.assertIn("document.documentElement.classList.remove('provider-drawer-open')", builtin_source)
        self.assertIn("drawerBackdrop.addEventListener('click', closeDrawer)", builtin_source)
        self.assertIn("if (event.key === 'Escape') closeDrawer()", builtin_source)
